...
import { createSubCategory, getAllCategories } from '../../services/category.service';
...
const validationSchema = yup.object().shape({
    ...
    imageURL: yup
        .string('Enter your lastName')
        .trim()
        .matches(/^[A-Za-z0-9]+$/, 'Image URL should only contain letters and numbers')
...
const CreateSubCategory = ({ open, handleClose }) => {
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            try {
                ...
                if(res && res?.statusCode == 200) {
                    setLoading(false);
                    resetForm();
                    toastAlert(res?.message ? res?.message : "Sub Category created successfully",'success')
                    handleClose();
                }else {
                    console.log('error adding your Data : ', res.response.data.message);
                    setLoading(false);
                    let message = res && res.response.data.message !== undefined ? res.response.data.message : "Problem while adding sub category Records.";
            ...
            <Box sx={style}>
                <Grid container>
                    <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                        <Grid container>
                            <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                                <span className='modal-header'>Create Sub Category</span>
                            </Grid>
                            <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                                <Autocomplete
                                    id="category"
