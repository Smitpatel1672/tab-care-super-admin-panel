...
import { editSubCategory, getAllCategories, getSubCategoryById } from '../../services/category.service';
...
const EditSubCategory = ({ open, handleClose, subCategoryId }) => {
    ...
    useEffect(() => {
        if(open) {
            // Fetch Sub category data by subCategoryId
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            setLoading(true);
            try {
                const res = await editSubCategory(values,subCategoryId);
                if(res && res?.statusCode == 200) {
                    setLoading(false);
                    resetForm();
                    toastAlert(res?.message ? res?.message : "Sub Category updated successfully",'success')
                    handleClose();
                }else {
                    console.log('error updating your Data : ', res.response.data.message);
                    setLoading(false);
                    let message = res && res.response.data.message !== undefined ? res.response.data.message : "Problem while updating sub category Records.";
    ...
    const getSingleSubCategoryData = async () => {
        try {
            ...
            if(subCategoryData && subCategoryData?.statusCode == 200) {
                formik.setValues({
                    category: categoryData?.find((category) => category.uuid === subCategoryData?.data?.category?.uuid),
                    title: subCategoryData.data.title,
                    desc: subCategoryData.data.description,
                    imageURL: subCategoryData.data.img,
                    isActive: subCategoryData.data.isActive,
                });
            } else {
                console.log('error fetching sub category data: ', subCategoryData, subCategoryData?.message);
                let message = subCategoryData && subCategoryData?.message !== undefined ? subCategoryData.message : 'Problem fetching sub category data.';
                toastAlert(message, 'error');
            }
        } catch (error) {
            console.error(error);
            ...
            <Box sx={style}>
                <Grid container>
                    <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                        <Grid container>
                            <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                                <span className='modal-header'>Edit Sub Category</span>
                            </Grid>
                            <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                                <Autocomplete
                                    id="category"
                                    ...
                                    renderInput={(params) => (
                                        <TextField
                                            ...
                                            variant="filled"
                                            value={formik.values.category?.title || null}
                                            inputProps={{
                                                ...params.inputProps,
