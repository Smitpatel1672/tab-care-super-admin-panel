...
import { categoryWiseSubCategory, editSubSubCategory, getAllCategories, getSubSubCategoryById } from '../../services/category.service';
...
const EditSubSubCategory = ({ open, handleClose, subSubCategoryId }) => {
    ...
    useEffect(() => {
        if(open) {
            // Fetch Sub sub category data by subSubCategoryId
            getSingleSubSubCategoryData();
        }
    }, [open, subSubCategoryId])
    ...
    const getSingleSubSubCategoryData = async () => {
        try {
            const subSubCategoryData = await getSubSubCategoryById(subSubCategoryId);
            console.log(subSubCategoryData,'subSubCategoryData');
            if(subSubCategoryData && subSubCategoryData?.statusCode == 200) {
                await fetchCategoryWiseSubCategory(subSubCategoryData?.data?.category?.uuid);
                formik.setValues({
                    category: categoryData?.length ? categoryData?.find((category) => category.uuid === subSubCategoryData?.data?.category?.uuid) : null,
                    subCategory: subCategoryData?.length ? subCategoryData?.find((subCategory) => subCategory.uuid === subSubCategoryData?.data?.subCategory?.uuid) : null,
                    title: subSubCategoryData.data.title,
                    desc: subSubCategoryData.data.description,
                    imageURL: subSubCategoryData.data.img,
                    isActive: subSubCategoryData.data.isActive,
                });
            } else {
                console.log('error fetching sub sub category data: ', subSubCategoryData, subSubCategoryData?.message);
                let message = subSubCategoryData && subSubCategoryData?.message !== undefined ? subSubCategoryData.message : 'Problem fetching sub sub category data.';
                toastAlert(message, 'error');
            }
        } catch (error) {
            console.error(error);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            setLoading(true);
            try {
                const res = await editSubSubCategory(values,subSubCategoryId);
                if(res && res?.statusCode == 200) {
                    setLoading(false);
                    resetForm();
                    toastAlert(res?.message ? res?.message : "Sub Sub Category updated successfully",'success')
                    handleClose();
                }else {
                    console.log('error updating your Data : ', res.response.data.message);
                    setLoading(false);
                    let message = res && res.response.data.message !== undefined ? res.response.data.message : "Problem while updating sub sub category Records.";
                    toastAlert(message,'error')
                }
            } catch (error) {
                console.error(error);
            ...
            <Box sx={style}>
                <Grid container>
                    <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                        <Grid container>
                            <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                                <span className='modal-header'>Edit Sub Sub Category</span>
                            </Grid>
                            <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                                <Autocomplete
                                    id="category"
