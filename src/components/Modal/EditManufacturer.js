...
import { editManufacturer, getManufacturerById } from '../../services/manufacturer.service';
...
const EditManufacturer = ({ open, handleClose, manufacturerId }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            setLoading(true);
            try {
                const res = await editManufacturer(values, manufacturerId);
                if(res && res?.statusCode == 200) {
                    setLoading(false);
                    resetForm();
                    toastAlert(res?.message ? res?.message : "Manufacturer updated successfully",'success')
                    handleClose();
                }else {
                    console.log('error adding your Data : ', res.response.data.message);
                    setLoading(false);
                    let message = res && res.response.data.message !== undefined ? res.response.data.message : "Problem while updating manufacturer Record.";
    ...
    const getManufacturerData = async () => {
        try {
            const manufacturerData = await getManufacturerById(manufacturerId);
            console.log(manufacturerData,'manufacturerData');
            if(manufacturerData && manufacturerData?.statusCode == 200) {
                formik.setValues({
                    name: manufacturerData.data.name,
                    address: manufacturerData.data.address,
                    isActive: manufacturerData.data.isActive,
                });
            } else {
                console.log('error fetching manufacturer data: ', manufacturerData, manufacturerData?.message);
                let message = manufacturerData && manufacturerData?.message !== undefined ? manufacturerData.message : 'Problem fetching manufacturer data.';
                toastAlert(message, 'error');
            }
        } catch (error) {
            console.error(error);
            ...
            <Box sx={style}>
                <Grid container>
                    <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                        <Grid container>
                            <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                                <span className='modal-header'>Edit Manufacturer</span>
                            </Grid>
                            <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                                <TextField
                                    id="name"
